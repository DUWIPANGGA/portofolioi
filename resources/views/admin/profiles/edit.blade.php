...
<div class="section">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex gap-3">
            <a href="{{ route('admin.users.show', $user) }}" class="neumorph-btn dark:neumorph-btn-dark px-4 py-2 rounded-xl hover:text-primary transition flex items-center justify-center whitespace-nowrap">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="phone" name="phone" value="{{ old('phone', $profile->phone ?? '') }}" class="w-full input-field dark:input-field px-4 py-2">
                        @error('phone')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="location" name="location" value="{{ old('location', $profile->location ?? '') }}" class="w-full input-field dark:input-field px-4 py-2">
                        @error('location')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        @endif
                        <input type="file" id="avatar" name="avatar" class="w-full input-field dark:input-field px-4 py-2">
                        @error('avatar')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mb-3">
                            <a href="{{ Storage::url($profile->cv_path) }}" target="_blank" class="text-primary hover:underline flex items-center">
                                <i class="fas fa-file-pdf mr-2"></i> View Current CV
                            </a>
                        </div>
                        @endif
                        <input type="file" id="cv" name="cv" class="w-full input-field dark:input-field px-4 py-2">
                        @error('cv')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <input type="url" id="social_{{ $platform }}" name="social_links[{{ $platform }}]" value="{{ old("social_links.$platform", $socialLinks[$platform] ?? '') }}" placeholder="https://{{ $platform }}.com/username" class="w-full input-field dark:input-field px-4 py-2 text-sm">
                            </div>
                            @endforeach
