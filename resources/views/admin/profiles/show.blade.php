...
<div class="section">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.profiles.edit', $user->profile) }}" 
                            ...
                            @foreach($profile->social_links as $platform => $url)
                                @if($url)
                                    <a href="{{ $url }}" target="_blank" 
                                       class="neumorph-btn dark:neumorph-btn-dark w-10 h-10 rounded-full flex items-center justify-center text-primary hover:bg-primary hover:text-white transition"
                ...
                @if(isset($profile->cv_path) && $profile->cv_path)
                    <div class="mt-6">
                        <a href="{{ Storage::url($profile->cv_path) }}" target="_blank" 
                           class="neumorph-btn dark:neumorph-btn-dark px-4 py-2 rounded-xl bg-primary text-white hover:bg-primary-dark transition inline-flex items-center">
