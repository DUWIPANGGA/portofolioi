        ...
        <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <div class="mb-4">
                        <img src="{{ Storage::url($post->featured_image) }}" alt="Current featured image" class="h-32 object-cover rounded-lg">
                        <label class="flex items-center mt-2">
                            <input type="checkbox" name="remove_featured_image" class="form-checkbox text-red-500">
                ...
                <div>
                    ...
                    <textarea name="excerpt" id="excerpt" rows="3" 
                              class="input-field dark:input-field w-full" required>{{ old('excerpt', $post->excerpt) }}</textarea>
                    @error('excerpt')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="10" 
                              class="input-field dark:input-field w-full" required>{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <div class="flex items-center space-x-4">
                            <label class="inline-flex items-center">
                                <input type="radio" name="is_published" value="1" 
                                       class="form-radio text-primary" {{ old('is_published', $post->is_published) == '1' ? 'checked' : '' }}>
                                <span class="ml-2">Published</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="radio" name="is_published" value="0" 
                                       class="form-radio" {{ old('is_published', $post->is_published) == '0' ? 'checked' : '' }}>
